...
\rmk{Observaciones sobre espacios localmente euclídeos}{
    1. Por ser \(X\) localmente euclídeo, este hereda las propiedades locales de \(\mathbb{R}^n\).  
    2. Podemos sustituir la bola abierta en la definición anterior por un entorno abierto de \(\mathbb{R}^n\).
}
...
\defn{Variedad topológica}{
    Una variedad topológica \(M\) es un espacio topológico \(\T_2\) y \(2AN\) que es localmente euclídeo. La dimensión de \(M\) es el número natural \(n\). También se denomina \(n\)-variedad (topológica).
}
...
\rmk{Observaciones sobre variedades topológicas}{
    1. Toda variedad topológica es localmente conexa por caminos (y localmente conexa).  
    ...
    6. Si cambiamos el espacio modelo \(\mathbb{R}^n\) por el semiespacio superior \(\mathbb{H}^n = \{x \in \mathbb{R}^n : x_n \geq 0\}\), obtenemos el concepto de variedad con borde.
}
...
\defn{Unión disjunta}{
    ...
    \]
    Consideramos las inyecciones canónicas \(u_\alpha : X_\alpha \to \bigsqcup_{\alpha \in J} X_\alpha\), dadas por \(u_\alpha(x) = (x, \alpha)\).
}

\defn{Topología unión disjunta}{
    La familia de subconjuntos \(B = \{u_\alpha(U) : U \in T_\alpha, \alpha \in J\}\) es una base para una topología \(T\) sobre \(\bigsqcup_{\alpha \in J} X_\alpha\) que recibe el nombre de topología unión disjunta.
}

\prop{Propiedades de la unión disjunta}{
    1. Cada inclusión \(\nu_{\alpha}\) es un embebimiento, por lo que podemos identificar \(X_{\alpha} = \nu_{\alpha}(X_{\alpha}) \subset \bigsqcup_{\alpha \in J} X_{\alpha}\).  
    2. Un subconjunto es abierto en \(\bigsqcup_{\alpha \in J} X_{\alpha}\) si y solo si su intersección con cada \(X_{\alpha}\) es un abierto en \(X_{\alpha}\).  
    3. Una aplicación \(f : \bigsqcup_{\alpha \in J} X_{\alpha} \to Y\) es continua si y solo si \(f|_{X_{\alpha}}\) es continua, para todo \(\alpha \in J\).  
    4. Si todos los espacios \(X_{\alpha}\) son \(T_2\) (resp. \(1AN\)), la unión disjunta también es \(T_2\) (resp. \(1AN\)).  
    5. Si todos los espacios \(X_{\alpha}\) son \(2AN\) y \(J\) es numerable, entonces la unión disjunta también es \(2AN\).  
    6. La unión disjunta de una cantidad numerable de \(n\)-variedades es una \(n\)-variedad.
}

\section{Suma conexa}

\defn{Suma conexa}{
    Sean \(S_1\) y \(S_2\) dos superficies conexas y sean \(D_1\) y \(D_2\) discos regulares euclídeos. Sea \(\varphi : \partial D_1 \to D_2\) un homeomorfismo y denotemos por \(S_i' := S_i \setminus D_i, \ i = 1, 2\). Definimos en \(S_1' \sqcup S_2'\) la menor relación de equivalencia que contiene a \(x \sim \varphi(x)\) para todo \(x \in \partial D_1\). Entonces el cociente \(S_1' \sqcup S_2' / \sim\) es un espacio topológico.
}

\thmr{Proposición 4.3}{
    Si \(S_1\) y \(S_2\) son dos superficies topológicas conexas, entonces, salvo homeomorfismo, el espacio \(S_1' \sqcup S_2' / \sim\) no depende de los discos regulares euclídeos ni del homeomorfismo \(\varphi\).
}
