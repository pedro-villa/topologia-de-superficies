...
\propp{Espacio proyectivo real}{
    ...
    Podemos dar una expresión explicita de las clases de equivalencia en $\mathbb{RP}^n$ como \newline $[x_0,\cdots,x_{n+1}]=\{(\lambda x_0,\cdots,\lambda x_{n+1}):\lambda\in\R\}$, Veamos que $\mathbb{RP}^n$ es localmente euclídeo. Sea $p=[x_0,\cdots,x_{n+1}]\in\mathbb{RP}^n$, por definición de $\mathbb{RP}^n$ sabemos que existe un $i_0$ tal que $x_{i_0} \neq 0$, definimos el siguiente entorno $U_p = \{[y_0,\cdots,y_{n+1}] : y_{i_0} \neq 0 \}$ que contiene a p y abierto puesto que $\pi^{-1}(U_p) = \{(y_1,\cdots,y_{n+1}) : y_{i_0} \neq 0\}$ es abierto ya que su complementario $\mathbb{R}^{n+1} \setminus \pi^{-1}(U_p) = \{(y_1,\cdots,y_{n+1}) : y_{i_0} = 0\} $ es cerrado, entonces la siguiente carta: $\varphi :U_p \rightarrow  \mathbb{R}^n$ con $\varphi([x_0:\cdots:x_{n+1}])=(\frac{x_0}{x_{i_0}},\cdots,\frac{x_{i_0-1}}{x_{i_0}},\frac{x_{i_0+1}}{x_{i_0}},\cdots,\frac{x_{n+1}}{x_{i_0}})$. Omitiremos ver que $\varphi$ esta bien definida y es un homeomorfismo.
}
