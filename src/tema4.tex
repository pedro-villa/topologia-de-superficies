...
\ex{
    ...
    \]
    Entonces $\faktor{X}{R}$ es localmente euclídeo de dimensión 1 pero no es Hausdorff, por tanto no es una variedad. Para más información consultar \href{https://en.wikipedia.org/wiki/Non-Hausdorff_manifold#Line_with_two_origins}{Line with two origins}.
}
...
\pf{
    ...
    Para ver que $\mathbb{T}^2$ es localmente euclídeo sea $p=(x,y)\in\mathbb{T}^2=\sphere^1\times\sphere^1$, de manera que
    \[
    p=((\cos\theta,\sin\theta),(\cos\phi,\sin\phi))
    \]
    para ciertos $\theta,\phi\in[0,2\pi)$. Consideremos la bola abierta $B=B((\theta,\phi),1)\subset\R^2$ y el homeomorfismo
    \[
...
\pf{
    Sea $p=(\cos\theta,\sin\theta,s)\in\sphere^1\times\R$ y consideremos la bola abierta $B=B((\theta,s),1)\subset\R^2$ y el homeomorfismo
    \[
...
\pf{
    Sea $P=\{(x,y,z)\in\R^3 \mid x^2 + y^2 - z = 0\}$ y sea $p\in P, p=(x,y,z)$. Consideremos el número $r=1+\sqrt{x^2+y^2}$ y la bola $B=B((x,y),r)$, así como el homeomorfismo
    \[
    \varphi:B\to U,\quad \varphi(x',y')=(x',y',x'^2+y'^2)
    \]
    definido en su imagen $U=\varphi(U)$, entonces $(U,\varphi^{-1})$ es una carta que cubre al punto $p$, lo que prueba que $P$ es una superficie.
}
