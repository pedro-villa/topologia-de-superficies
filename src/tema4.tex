...
Finalmente por la definición de topología cociente debe de ser $p(U)\in\faktor{\T}{K}$, pero como además $p|_{X\setminus K}=p(U)=p(U)\cap p(X\setminus K)$ deducimos que $p|_{X\setminus K}(U)\in\faktor{\T}{K}|_{p(X\setminus K)}$, lo que prueba que $p|_{X\setminus K}$ es abierta y por tanto un homeomorfismo.
...
\pf{
    ...
    y también
    \[
    F({\partial B'})=\tilde{\varphi}(\phi(\tilde{\varphi}^{-1}(\partial B')))=\tilde{\varphi}(\phi(\partial B))=\tilde{\varphi}(\partial B)=\partial B'\implies F|_{\partial B'}=Id.
    \]
