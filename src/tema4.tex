...
\propp{Espacio proyectivo real}{
    ...
    \[
        [x_0,\cdots,x_{n+1}]=\{(\lambda x_0,\cdots,\lambda x_{n+1}):\lambda\in\R\setminus\{0\},\exists x_{i_0}\neq 0\}
    \]
    Veamos que $\mathbb{RP}^n$ es localmente euclídeo. Sea $p=[x_0,\cdots,x_{n+1}]\in\mathbb{RP}^n$, por definición de $\mathbb{RP}^n$ sabemos que existe un $i_0$ tal que $x_{i_0} \neq 0$, definimos el siguiente entorno $U_p = \{[y_0,\cdots,y_{n+1}] : y_{i_0} \neq 0 \}$ que contiene a $p$ y es abierto puesto que es la preimagen a través de la proyección $\pi:\R^{n+1}\setminus\{0\}\to\mathbb{RP}^n$ del conjunto
    \[
    ...
    \]
    es cerrado. Como $\pi$ es continua $U_p=\pi^{-1}(V_p)$ es abierto. Entonces la siguiente carta: $\varphi :U_p \rightarrow  \mathbb{R}^n$ con $\varphi([x_0:\cdots:x_{n+1}])=(\frac{x_0}{x_{i_0}},\cdots,\frac{x_{i_0-1}}{x_{i_0}},\frac{x_{i_0+1}}{x_{i_0}},\cdots,\frac{x_{n+1}}{x_{i_0}})$ es un homeomorfismo. Omitiremos ver que $\varphi$ esta bien definida y es un homeomorfismo.

    %TODO Justificar que es T2 y Hausdorff.
}
...
\propp{Invarianza del cociente \(\faktor{S_1' \sqcup S_2'}{\sim}\)}{\label{prop:invariancia-cociente}
