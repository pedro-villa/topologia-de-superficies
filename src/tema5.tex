...
\pf{
    ...
    \[
        ...
        \begin{cases}
            ...
            & \text{presentación de } \R\mathbb{P}^2\#\dots\#\R\mathbb{P}^2.
        \end{cases}
    \]
    ...
    \noindent
    Dado que la superficie es conexa, si $\partes$ tiene varias palabras, para cada palabra debe existir una arista que se identifique con una arista de otra palabra. 
    ...
    Pero entonces la superficie es una esfera.
    ...
    Rotamos y pegamos por $a$ ($bVa$ y $a^{-1}W^{-1}b$). Entonces llegamos a $bbVW^{-1}$. 
    ...
    \noindent
    Si no fuese así, tendríamos $aXa^{-1}Y$, donde además $X$ e $Y$ no comparten aristas 
    (si compartiesen alguna letra $b$, esta debería aparecer como $b$ tanto en $X$ como en $Y$, 
    lo cual no es posible por el paso 3). 
    Esto significa también que $X$ e $Y$ no comparten vértices, ya que si $x$ fuese un vértice de $X$ y $y$ un vértice de $Y$, 
    habría una arista a la vez en $X$ y en $Y$, contradiciendo así que $X$ e $Y$ no comparten aristas.
    ...
    El dibujo que hizo en clase con las $c$'s es muy instructivo para ver esto. 
    Si alguien lo tiene copiado y lo pone como imagen sería maravilloso (una imagen vale más que mil palabras).

    \noindent
    \textcolor{blue}{¡ATENCIÓN!} 
    Me uno al ruego del compañero, incluso invito al lector a hacer el dibujo y verlo por sí mismo, en este caso no es difícil.
    ...
    y hacemos transformaciones para que las aristas aparezcan consecutivas. 
    Primero describimos los pasos y luego los escribimos de forma simbólica.

    \vspace{0.5em}
    \noindent
    Cortamos desde el final de $X$ al final de $W$, y llamamos a esa arista $c$. 
    Vamos a pegar por $a$. 
    Tendremos $WaXc$ y $c^{-1}bYa^{-1}Zb^{-1}$. 
    Rotando de buena manera y pegando se queda $XcWzb^{-1}c^{-1}bY$. 

    \vspace{0.5em}
    \noindent
    Ahora cortamos desde el final de $b^{-1}$ al final de $c$, llamando $d$ a esa arista. 
    Se nos queda $c^{-1}bYXcd$ y $d^{-1}WZb^{-1}$. 
    Rotando y pegando por $b$ se nos queda $YXcd^{-1}c^{-1}dWZ$.

    \vspace{0.5em}
    \noindent
    \begin{align*}
        aXbYa^{-1}Zb^{-1}W 
        &\cong WaXbYa^{-1}Zb^{-1} \;\;\cong\;\; WaXc \text{, } c^{-1}bYa^{-1}Zb^{-1}\\
        &\cong XcWa \text{, } a^{-1}Zb^{-1}c^{-1}bY 
        \;\;\cong\;\; XcWZb^{-1}c^{-1}bY\\
        &\cong c^{-1}bYXcWZb^{-1} 
        \;\;\cong\;\; c^{-1}bYXcd \text{, } d^{-1}WZb^{-1}\\
        &\cong YXcdc^{-1}b \text{, } b^{-1}d^{-1}WZ 
        \;\;\cong\;\; YXcdc^{-1}d^{-1}WZ
    \end{align*}
    ...
    la superficie sería homeomorfa a un toro o a un plano proyectivo, respectivamente.
    ...
    Es decir, $aba^{-1}b^{-1}cc \cong ddeeff$. 
    Iterando sobre cada par (toro, proyectivo) podemos eliminar los toros y quedarnos con un número finito de planos proyectivos.
