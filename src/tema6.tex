...
\propp{}{
    ...
    Sea $[\alpha]\in\pi_1(X,x_0)$, entonces
    \[
        \hat{\sigma} \circ f_{*}([\alpha]) = [\overline{\sigma}] * [f \circ \alpha] * [\sigma] = [\tau * f \circ \alpha * \overline{\tau}] = [g \circ \alpha] = g_*([\alpha])
    \]
    donde hemos usado que $f\circ\alpha,g\circ\alpha$ son lazos en torno al mismo punto y además $\sigma=\tau$ igual que en el lema anterior.
}
...
\corp{
    ...
    Si $X$ es contractial entonces es homotópico a un punto $\{x\}$, como el único lazo en este espacio es el lazo constante, su grupo fundamental es el grupo trivial: $\pi_1(\{x\})\cong(0,+)$. Finalmente, por el corolario anterior el grupo fundamental es un invariante homotópico, y como $X\simeq\{x\}$ hemos de tener
    \[
...
\pf{
    ...
    Además, para cualquier $b \in \mathbb{S}^1$, podemos tomar $U = \mathbb{S}^1 \setminus \{-b\}$. Entonces:
    \[
        p^{-1}(U) = V_1 \cup V_2,
    \]
    donde $V_1$ y $V_2$ son dos hemisferios abiertos de $\mathbb{S}^1$. Cada $V_i$ es la preimagen de $U$ en un "sector" de $\mathbb{S}^1$ que cubre la mitad del círculo, y $p|_{V_i}$ es un homeomorfismo.
}
